...
class weberp {
	...
	function authenticate() {
		$this->db->Execute('SET search_path=company'.EGS_COMPANY_ID);
		...
		if(isset($_SESSION['AttemptsCounter'])) $_SESSION['AttemptsCounter']++;
		else $_SESSION['AttemptsCounter'] = 0;
		...
		$query = "SELECT www_users.fullaccess,
							...
							www_users.language
						FROM www_users
						WHERE www_users.userid=".$this->db->qstr(EGS_USERNAME)." 
						AND www_users.password=".$this->db->qstr($this->db->GetOne('SELECT password FROM public.users WHERE username='.$this->db->qstr(EGS_USERNAME)));
		...
		while (!$rs->EOF) {
			if ($rs->fields['blocked'] != 1) {
				...
				if ($rs->fields['displayrecordsmax'] > 0) {
					$_SESSION['DisplayRecordsMax'] = $rs->fields['displayrecordsmax'];
				} else {
					$_SESSION['DisplayRecordsMax'] = $_SESSION['preferences']['perpage'];
				}

				$sql = "UPDATE www_users SET lastvisitdate='".date("Y-m-d H:i:s")."' 
											WHERE www_users.userid='".$this->db->qstr(EGS_USERNAME)."' 
											AND www_users.password='".$this->db->qstr($this->db->GetOne('SELECT password FROM public.users WHERE username='.$this->db->qstr(EGS_USERNAME)))."'";
				//$Auth_Result = DB_query($sql, $this->db);
		...
		$this->db->Execute('SET search_path=public');
	}
}
?>
