...
//
//$noneareset = false;
//if(!isset($_SESSION['preferences']['homePreferences'])) {$noneareset = false; echo 'None set';}

if(isset($_GET['do']) && ($_GET['do'] == 'deletemessage') && isset($_GET['id'])) $egs->DeleteMessage($_GET['id']);
else if(isset($_GET['do']) && ($_GET['do'] == 'completeactivity') && isset($_GET['id'])) {
...
//$homePrearray = $_SESSION['preferences']['homePreferences'];
...
if(sizeof($news) > 0) $smarty->assign('news', $news);
}
...
$query = 'SELECT id, '.$db->SQLDate(str_replace('%', '', EGS_DATE_FORMAT), 'published').' AS publisheddate, headline FROM news n LEFT OUTER JOIN domain d ON (n.d WHERE companyid='.$db->qstr(EGS_COMPANY_ID).' AND news=false AND motd=false AND domainid IS NULL AND visible=true AND (showfrom<=now() OR showfrom IS NULL) AND (showuntil>=now() OR showuntil IS NULL) ORDER BY published DESC LIMIT 10';
...
if(sizeof($announcements) > 0) $smarty->assign('announcements', $announcements);
...
if(sizeof($tickets) > 0) $smarty->assign('tickets', $tickets);
...
if(sizeof($projects) > 0) $smarty->assign('projects', $projects);
...
if(isset($_SESSION['preferences']['homePreferences']) && in_array('to_do', $_SESSION['preferences']['homePreferences']) && file_exists(EGS_FILE_ROOT.'/modules/calendar/calendars/'.EGS_COMPANY_ID.'/'.EGS_USERNAME.'.ics')){
	...
	if(isset($temp))$_COOKIE['phpicalendar']=$temp;
	//print_r($master_array[-2]);
	...
	if (is_array($master_array['-2'])) {
				...
				foreach ($master_array['-2'] as $vtodo_times) {
					...
					foreach ($vtodo_times as $key=>$val) {
						...
						if($status!='COMPLETED')$todos[]=array('link'=>$link,'name'=>$vtodo_text,'array'=>$vtodo_array,'deadline'=>$due_date,'priority'=>$temp);
...
if(isset($_SESSION['preferences']['homePreferences']) && in_array('events', $_SESSION['preferences']['homePreferences']) && file_exists(EGS_FILE_ROOT.'/modules/calendar/calendars/'.EGS_COMPANY_ID.'/'.EGS_USERNAME.'.ics')){
	...
	if(!is_numeric($_SESSION['preferences']['eventsDisplay'])) unset($_SESSION['preferences']['eventsDisplay']);
	if(!isset($_SESSION['preferences']['eventsDisplay'])) $_SESSION['preferences']['eventsDisplay']=14;
	...
	//echo '<script type="text/javascript" src="'.EGS_SERVER.'/modules/calendar/functions/event.js"></script>';
			...
			foreach($event_array as $event_key=>$event_data) {
				...
				if($owner==EGS_USERNAME || $show) {
					...
					/*don't want projects, but all day events are -1 too*/
						...
						if(!strpos($event_key,'PROJECT')) {
						
						//$text = openevent($getdate,$uid,$event_data);
