...
if (isset ($_SESSION['modules']) && (in_array('home', $_SESSION['modules'])) && (EGS_COMPANY_ID == EGS_ACTUAL_COMPANY_ID)) {	
	...
	$newsDetails = $db->GetRow($query);
	if(!$newsDetails = $db->GetRow($query))
		$errors=_('You don\'t have permission to view this Announcement');
	if(count($errors)==0) {	
	$smarty->assign('pageTitle', _('View Announcement'));
	
		/* Output the announcement details */
		$leftData = array ();
		$leftData[] = array ('tag' => _('Headline'), 'data' => $newsDetails['headline']);
		...
		$bottomData = array();
		$bottomData[] = array ('type'=>'display','title'=>_('Full Announcement'),'content'=>$newsDetails['body']);	
