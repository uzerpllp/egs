		...
		foreach ($day_array as $key) {
			...
			for ($week_loop=0; $week_loop<7; $week_loop++) {
				...
				if (sizeof($event_length[$thisday]) == 0) {
					...
					$weekdisplay .= '<td colspan="' . $nbrGridCols . '" '.$class.' onClick="parent.location.href=\''.EGS_SERVER.'?module=calendar&amp;action=saveevent&amp;start='.$thisday.$cal_time.'\';" style="cursor:pointer;cursor:hand">&nbsp;</td>'."\n";
				} else {
	...
	function draw_day($template_p) {
		...
		foreach ($day_array as $key) {
			...
			if (sizeof($event_length) == 0) {
				//egs added this line
				$daydisplay .= '<td colspan="' . $nbrGridCols . '" '.$class.' onClick="parent.location.href=\''.EGS_SERVER.'?module=calendar&amp;action=saveevent&amp;start='.$this_year.$this_month.$this_day.$time.'\';" style="cursor:pointer;cursor:hand">&nbsp;</td>'."\n";
