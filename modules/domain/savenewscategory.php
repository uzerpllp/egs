<?php   
// +----------------------------------------------------------------------+
// | Enterprise Groupware System (EGS) - News Category 1.0                |
// +----------------------------------------------------------------------+
...
if (sizeof($_POST) > 0) {
	...
	if(isset($_POST['delete'])) $saved = $domain->deleteNewsCategory($_POST['id'], $_POST['domainid']);
	else $saved = $domain->saveNewsCategory($_POST, $id);
}
...
if ($saved) {
	...
	if (isset ($id)) {
		...
		if (isset ($id) && ($domain->accessLevel($domainId) > 0)) {
			$query = 'SELECT * FROM newscategory WHERE id='.$db->qstr($id).' AND domainid='.$db->qstr($domainId);
			...
			$smarty->assign('errors', array (_('You do not have the correct access to edit this news category. If you believe you should please contact your system administrator')));
			$smarty->assign('redirect', true);
	...
	/* Set up the title */
	if (isset ($id))
		$smarty->assign('pageTitle', _('Save Changes to News Category'));
	else
		$smarty->assign('pageTitle', _('Save New News Category'));
	...
	$query = 'SELECT id, name FROM newscategory WHERE domainid='.$db->qstr($domainId).' AND id<>'.intval($db->qstr($id)).' ORDER BY name';
